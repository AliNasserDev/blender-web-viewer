...
function worker_code () {
    ...
    // [blend-viewer patch] Blender >= 4.x can store array fields at byte offsets
    // that are not aligned to the element size, which makes direct TypedArray
    // views throw. Fall back to DataView reads there. Supported .blend files
    // are little-endian in practice, so we always decode as such (the
    // template.endianess flag is unreliable and is intentionally ignored).
    function makeTypedArray (TypedArr, AB, byteOffset, length /* , _endianess */) {
        const elemSize = TypedArr.BYTES_PER_ELEMENT;
        if ((byteOffset % elemSize) === 0) {
            return new TypedArr(AB, byteOffset, length);
        }
        const out = new TypedArr(length);
        const dv = new DataView(AB);
        let get;
        if (TypedArr === Float64Array) get = (i) => dv.getFloat64(i, true);
        else if (TypedArr === Float32Array) get = (i) => dv.getFloat32(i, true);
        else if (TypedArr === Int32Array) get = (i) => dv.getInt32(i, true);
        else if (TypedArr === Uint32Array) get = (i) => dv.getUint32(i, true);
        else if (TypedArr === Int16Array) get = (i) => dv.getInt16(i, true);
        else if (TypedArr === Uint16Array) get = (i) => dv.getUint16(i, true);
        else get = (i) => dv.getUint8(i);
        for (let i = 0; i < length; i++) out[i] = get(byteOffset + i * elemSize);
        return out;
    }
    ...
    function parseFile (msg) {
        ...
        if (typeof msg.data == 'object') {
            ...
            finished_objects = [];
            current_SDNA_template = null;
            // [blend-viewer patch] never reuse cached SDNA templates across files;
            // a failed/partial parse would otherwise poison later parses.
            templates = {};
        ...
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Float64Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
    ...
    function floatProp (offset, Blender_Array_Length, length) {
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Float32Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
    ...
    function intProp (offset, Blender_Array_Length, length) {
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Int32Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
    ...
    function uIntProp (offset, Blender_Array_Length, length) {
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Uint32Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
    ...
    function shortProp (offset, Blender_Array_Length, length) {
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Int16Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
    ...
    const uShortProp = (offset, Blender_Array_Length, length) => {
        return {
            get: function () {
                return (Blender_Array_Length > 1) ?
                    makeTypedArray(Uint16Array, this.__blender_file__.AB, this.__data_address__ + offset, length, this.__blender_file__.template.endianess) :
